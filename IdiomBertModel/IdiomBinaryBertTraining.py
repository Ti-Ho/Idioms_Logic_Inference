...
class IdiomTrainer:
    def __init__(self, max_seq_len,
        ...
        # 根据cls_type确定训练并列关系模型还是转折关系模型
        # 根据ifPool决定训练使用CLS判断还是mean max pool判断
        if self.cls_type == 1:
            if self.ifPool:
                self.state_dict_dir = self.config["bi_pool_state_dict_dir"]
            else:
                self.state_dict_dir = self.config["bi_cls_state_dict_dir"]
        if self.cls_type == 2:
            if self.ifPool:
                self.state_dict_dir = self.config["bi_pool_state_dict_dir2"]
            else:
                self.state_dict_dir = self.config["bi_cls_state_dict_dir2"]
    ...
    def iteration(self, epoch, data_loader, train=True, df_name="df_log.pickle"):
        ...
        df_path = self.state_dict_dir + "/" + df_name
...
if __name__ == "__main__":
    ...
    for epoch in range(start_epoch, start_epoch + train_epoches):
        if epoch == start_epoch and epoch == 0:
            ...
            trainer.load_model(trainer.bert_model, dir_path=trainer.state_dict_dir)
        print("train with learning rate {}".format(str(dynamic_lr)))
        # 训练一个epoch
        trainer.train(epoch)
        # 保存当前epoch模型参数
        trainer.save_state_dict(trainer.bert_model, epoch,
                                state_dict_dir=trainer.state_dict_dir,
